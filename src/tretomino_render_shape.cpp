...
void TretominoRenderShape::set_tretomino(Tretomino tretomino) {
    for (int i = 0; i < 4; i++) {
        ...
        float x_pos = (static_cast<float>(x_block) * REC_SIZE) + TRETOMINO_RENDER_OFFSET.at(tretomino).x;
        float y_pos = (static_cast<float>(y_block) * REC_SIZE) + TRETOMINO_RENDER_OFFSET.at(tretomino).y;
...
TretominoRenderShape::TretominoRenderShape() {
    for (sf::RectangleShape& shape : shape) {
        shape.setSize({ REC_SIZE - BETWEEN_CELL_LINE_THICKNESS, REC_SIZE - BETWEEN_CELL_LINE_THICKNESS, });
